  ...
  <div class="container px-6 mx-auto grid">
    ...
    <div class="flex justify-between text-purple-600 mb-4">
      <div>
        ...
        >
          <span>Tambah Penduduk Pindah</span> &nbsp;&nbsp;&nbsp;
          <svg class="w-6 h-6"
               fill="none"
               stroke="currentColor" 
               viewBox="0 0 24 24" 
        ...
        >
            ...
            <label class="block mt-4 text-sm">
              ...
              >
                ...
                @foreach ($movers as $mover)
                    <option value="{{ $mover->id }}">{{ $mover->villager ? $mover->villager->name : '-' }}</option>
                @endforeach
              </select>
            </label>
          </div>
          ...
          >
            <button
              ...
              formaction="{{ route('move.filter-reset') }}"
              >
              Reset
            </button>
            <button
              class="w-full px-5 py-3 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg sm:w-auto sm:px-4 sm:py-2 active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple"
              formaction="{{ route('move.index') }}"
            >
              Accept
            </button>
          </footer>
        ...
        <table class="w-full whitespace-no-wrap" id="data-table">
          <thead>
            ...
            >
              ...
              <th class="px-4 py-3">Tanggal Pindah</th>
          ...
          >
            <tr class="text-gray-700 dark:text-gray-400">
              ...
              <td class="px-4 py-3">
                <p class="text-sm text-gray-600 dark:text-gray-400">
                  {{ $mover->villager ? $mover->villager->name : '' }}
                </p>
              </td>
              <td class="px-4 py-3">
                <p class="text-sm text-gray-600 dark:text-gray-400">
                  {{ $mover->villager ? $mover->villager->id_number : '' }}
                </p>
              </td>
              <td class="px-4 py-3">
                <p class="text-sm text-gray-600 dark:text-gray-400">
              ...
              <td class="px-4 py-3 flex flex-column">
                ...
                <a class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-red-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" href="move/{{ $mover->id }}/confirm">
                  <svg
                    class="w-5 h-5"
                    aria-hidden="true"
                    fill="currentColor"
                ...
                <form action="{{ route('move.destroy', $mover->id) }}" method="POST">
                  @csrf
                  @method('DELETE')
