        ...
        >
            ...
            <label class="block mt-4 text-sm">
              ...
              >
                ...
                @foreach ($comers as $comer)
                    <option value="{{ $comer->id }}">{{ $comer->name }}</option>
                @endforeach
              </select>
            </label>
          ...
          >
            <button
              ...
              formaction="{{ route('comer.filter-reset') }}"
              >
              Reset
            </button>
            <button
              class="w-full px-5 py-3 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg sm:w-auto sm:px-4 sm:py-2 active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple"
              formaction="{{ route('comer.index') }}"
            >
              Accept
            </button>
          </footer>
          ...
          <table class="w-full whitespace-no-wrap" id="data-table">
            <thead>
              ...
              >
                ...
                <th class="px-4 py-3">Tanggal Tiba</th>
                <th class="px-4 py-3">Tanggal Lahir</th>
                <th class="px-4 py-3">L/P</th>
                <th class="px-4 py-3">Action</th>
              </tr>
            </thead>
            @foreach ($comers as $index=>$comer)
            <tbody
              ...
              <tr class="text-gray-700 dark:text-gray-400">
                ...
                <td class="px-4 py-3 flex flex-column">
                  <a
                    ...
                    aria-label="Edit"
                    href="{{ route('comer.edit', $comer->id) }}"
                  >
                    <svg
                      class="w-5 h-5"
                      aria-hidden="true"
                  ...
                  <a
                    class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                    aria-label="Show"
                    href="{{ route('comer.show', $comer->id) }}"
                  >
                  ...
                  <a class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-red-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" href="comer/{{ $comer->id }}/confirm">
                    <svg
                      class="w-5 h-5"
                      aria-hidden="true"
                      fill="currentColor"
                  ...
                  <form action="{{ route('comer.destroy', $comer->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
