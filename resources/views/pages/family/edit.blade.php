...
@section('content')
  <main class="h-full pb-16 overflow-y-auto">
    <div class="container px-6 mx-auto grid">
      ...
      >
        <form action="{{ route('family.update', $families->id) }}" method="POST" enctype="multipart/form-data">
          @csrf
          @method('PUT')
          ...
          <label class="block mt-4 text-sm">
            ...
            >
              @foreach ($patriarches as $patriarch)
                  <option value="{{ $patriarch->id }} {{ $patriarch->id === $families->villager_id ? 'selected' : '' }}">{{ $patriarch->name }}</option>
              @endforeach
            </select>
          </label>
